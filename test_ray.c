...
int	init_delta(t_scene *scene)
{
	...
	printf("delta x = %f, delta y = %f\n", scene->player.delta.x, scene->player.delta.y);
	return (0);
}
...
int	dda_hit(t_scene *scene)
{
	...
	while (hit == 0)
	{
		...
		// if (scene->player.side == 0)
	// 	scene->player.perpwdist = fabs((scene->player.x_f - (scene->player.rposx) + (1 - scene->player.stepx) / 2) / (scene->player.dir.x));
	// else
	// 	scene->player.perpwdist = fabs((scene->player.y_f - (scene->player.rposy) + (1 - scene->player.stepy) / 2) / (scene->player.dir.y));
